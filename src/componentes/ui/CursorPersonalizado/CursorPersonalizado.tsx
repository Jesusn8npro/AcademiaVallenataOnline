...
const CursorPersonalizado: React.FC = () => {
    ...
    useEffect(() => {
        ...
        const contenedorPortal = document.createElement('div');
        contenedorPortal.className = 'cursor-personalizado-layer';
        contenedorPortal.setAttribute('data-cursor-layer', 'true');

        const asegurarCursorAlFrente = () => {
            if (contenedorPortal.parentElement === document.body && document.body.lastElementChild !== contenedorPortal) {
                document.body.appendChild(contenedorPortal);
            }
        };

        document.body.appendChild(contenedorPortal);
        asegurarCursorAlFrente();
        setPortalContainer(contenedorPortal);
        ...
        return () => {
            ...
            if (contenedorPortal.parentElement) {
                contenedorPortal.parentElement.removeChild(contenedorPortal);
            }
