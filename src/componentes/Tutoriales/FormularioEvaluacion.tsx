    ...
    return (
      <div className="evaluacion-status-container">
        <div className={`evaluacion-status-card estado-${evaluacion.estado}`}>
          ...
          <button
            type="button"
            className="evaluacion-ver-video-btn"
          ...
          {videoAbierto && (
            <div className="evaluacion-video-wrapper">
              {embed.tipo === 'iframe' ? (
                <iframe
                  src={embed.src}
                  className="evaluacion-video-player"
                  allowFullScreen
                  title="Video enviado"
  ...
  return (
    <div className="evaluacion-form-container">
      ...
      <div className="evaluacion-pasos">
        <div className="evaluacion-paso">
          <span className="evaluacion-paso-num">1</span>
          <div>
            <strong>Graba tu ejecución</strong>
            <span>Toca la pieza completa del tutorial, de inicio a fin.</span>
          </div>
        </div>
        <div className="evaluacion-paso">
          <span className="evaluacion-paso-num">2</span>
          <div>
            <strong>Envía tu video</strong>
            <span>Súbelo directo o comparte el enlace de YouTube / Google Drive.</span>
          </div>
        </div>
        <div className="evaluacion-paso">
          <span className="evaluacion-paso-num">3</span>
          <div>
            <strong>Revisión profesional</strong>
            <span>Un profesor lo revisa en las próximas horas y te envía retroalimentación.</span>
          </div>
        </div>
        <div className="evaluacion-paso">
          <span className="evaluacion-paso-num">4</span>
          <div>
            <strong>Gana tus recompensas</strong>
            <span>Si todo está bien, recibes monedas, XP y el 100% del tutorial completado.</span>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={enviar} className="evaluacion-form">
        <div className="evaluacion-modo-tabs">
          <button
            type="button"
            className={`evaluacion-modo-tab ${modoEnvio === 'archivo' ? 'activo' : ''}`}
            onClick={() => { setModoEnvio('archivo'); setError('') }}
          >
            📁 Subir Video Directo
          </button>
          <button
            type="button"
            className={`evaluacion-modo-tab ${modoEnvio === 'url' ? 'activo' : ''}`}
            onClick={() => { setModoEnvio('url'); setError('') }}
          >
            🔗 YouTube / Drive
          </button>
        </div>
