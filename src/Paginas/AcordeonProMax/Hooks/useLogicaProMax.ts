...
export function useLogicaProMax() {
  ...
  const reset = useCallback(() => {
    // 1. Limpiar completamente todo el estado (estadísticas)
    setEstadisticas({ ...ESTADISTICAS_INICIALES });
    estadisticasRef.current = { ...ESTADISTICAS_INICIALES };
    
    // 2. Limpiar control de flujo (tiempoInicio, juegoIniciado, juegoTerminado representados por estadoJuego)
    setEstadoJuego('seleccion');
    estadoJuegoRef.current = 'seleccion';
    
    // 3. Limpiar variables extras de juego (notas jugadas, totales, etc)
    notasImpactadasRef.current = new Set();
    setNotasEsperando([]);
    notasEsperandoRef.current = [];
    setBotonesGuiaAlumno({});
    ...
    // 4. Limpiar buffers de audio
    motorAudioPro.detenerTodo();
    if (typeof (motorAudioPro as any).reset === 'function') {
        (motorAudioPro as any).reset();
    }
    if (typeof (motorAudioPro as any).stop === 'function') {
        (motorAudioPro as any).stop();
    }
    
    // Limpiar reproductor y loops
    reproductor.detenerReproduccion();
    reproductor.setLoopPoints(0, 0, false);
    actualizarLoopAB({ start: 0, end: 0, activo: false, hasStart: false, hasEnd: false });
    
    // Limpiar audio de fondo
    if (audioFondoRef.current) {
        audioFondoRef.current.pause();
    ...
    cancelarCapturaActiva();
    limpiarEstadoGrabaciones();
  }, [actualizarLoopAB, cancelarCapturaActiva, limpiarEstadoGrabaciones, reproductor]);
