...
const ArchivoAudio F1_Halar[16] = {
    ...
    {"/Brillante/Bb - 5-cm.wav"}, {"/Brillante/D - 6-cm.wav"}};
...
const ArchivoAudio B1_Empujar[16] = {
    {"/Bajos/Bajo Eb  (acorde)-cm.wav"}, {"/Bajos/Bajo  Eb-cm.wav"},
    {"/Bajos/Bajo Eb  (acorde)-cm.wav"}, {"/Bajos/Bajo G-cm.wav"},
    {"/Bajos/Bajo Ab  (acorde)-cm.wav"}, {"/Bajos/Bajo Ab-cm.wav"}};
const ArchivoAudio B1_Halar[16] = {
    {"/Bajos/Bajo Eb  (acorde)-cm.wav"}, {"/Bajos/Bajo G-cm.wav"},
    {"/Bajos/Bajo Cm (acorde)-cm.wav"},  {"/Bajos/Bajo C -cm.wav"},
    {"/Bajos/Bajo Ab  (acorde)-cm.wav"}, {"/Bajos/Bajo Ab-cm.wav"}};
const ArchivoAudio B2_Empujar[16] = {
    {"/Bajos/Bajo Eb  (acorde)-cm.wav"}, {"/Bajos/Bajo F-cm.wav"},
    {"/Bajos/Bajo Bb  (acorde)-cm.wav"}, {"/Bajos/Bajo Bb-cm.wav"},
    {"/Bajos/Bajo Eb  (acorde)-cm.wav"}, {"/Bajos/Bajo  Eb-cm.wav"}};
const ArchivoAudio B2_Halar[16] = {
    {"/Bajos/Bajo C  (acorde)-cm.wav"},  {"/Bajos/Bajo C -cm.wav"},
    {"/Bajos/Bajo Eb  (acorde)-cm.wav"}, {"/Bajos/Bajo F-cm.wav"},
    {"/Bajos/Bajo Bb  (acorde)-cm.wav"}, {"/Bajos/Bajo Bb-cm.wav"}};

void sonarNota(int m, int b) {
  // 1. GUARDIA ANTI-DOBLE NOTA: Si ya suena y no está en release, NO
  // re-disparar
  int iExistente = buscarVoz(m, b);
  if (iExistente != -1) {
    if (!voces[iExistente].enRelease)
      return; // Ya está sonando, ignorar rebote
    liberarVoz(
        iExistente); // Si estaba en release, reiniciamos para el vallenato
  }
  ...
  if (h == 1)
    ...
    a = ext ? (fuelleAbriendo ? &B2_Halar[nb] : &B2_Empujar[nb])
            : (fuelleAbriendo ? &B1_Halar[nb] : &B1_Empujar[nb]);
  ...
  for (int v = 0; v < NUM_VOCES; v++) {
    if (voces[v].ocupada && voces[v].wav && voces[v].wav->isRunning()) {
      ...
      if (voces[v].enAtaque) {
        ...
        if (voces[v].volumenActual >= VOL_MAX) {
          voces[v].volumenActual = VOL_MAX;
          voces[v].enAtaque = false;
        }
