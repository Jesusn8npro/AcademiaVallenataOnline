...
// TONALIDAD 0: "F-Bb-Eb"  (Fa - Sib - Mib) 
const offset_original = 0;
...
const NOTAS_ES = ['Do', 'Reb', 'Re', 'Mib', 'Mi', 'Fa', 'Solb', 'Sol', 'Lab', 'La', 'Sib', 'Si'];
const NOTAS_EN = ['C', 'Db', 'D', 'Eb', 'E', 'F', 'Gb', 'G', 'Ab', 'A', 'Bb', 'B'];

const wavsBrillanteLibpios = [
    "A-4-cm", "A-5-cm", "Ab-4-cm", "Ab-5-cm", "Ab-6-cm", "B-4-cm", "Bb-3-cm",
    "Bb-4-cm", "Bb-5-cm", "Bb-6-cm", "C-4-cm", "C-5-cm", "C-6-cm", "C-7-cm",
    "D-4-cm", "D-5-cm", "D-6-cm", "Db-5-cm", "Db-6-cm", "E-4-cm", "E-5-cm",
    "Eb-4-cm", "Eb-5-cm", "Eb-6-cm", "F-4-cm", "F-5-cm", "F-6-cm", "G-4-cm",
    "G-5-cm", "G-6-cm", "Gb-4-cm", "Gb-5-cm"
];

const wavsBajosLimpios = [
    "BajoEb(acorde)-cm", "BajoEb-cm", "BajoF-2-cm", "BajoF-cm",
    "BajoG-2-cm", "BajoG-cm", "BajoAb(acorde)-cm", "BajoAb-2-cm",
    "BajoAb-cm", "BajoBb(acorde)-cm", "BajoBb-cm", "BajoC(acorde)-cm",
    "BajoC-cm", "BajoCm(acorde)-cm", "BajoDb(acorde)-cm" // Agrega los faltantes si no están igual
];


function convertirAMidi(notaEng, octava) {
    let base = NOTAS_EN.indexOf(notaEng);
...
function archivoWavMasCercanoPitos(midiNote) {
    ...
    let wavMidis = wavsBrillanteLibpios.map(w => {
        let parts = w.split('-');
        let n = parts[0];
...
function generarArrayPitosC(fila, tipoFuelle, semitonosOffset) {
    ...
    for (let i = 1; i <= 16; i++) {
        ...
        if (n) {
            // Nota original (Ya en Fa-Sib-Mib)
            let oldIdx = NOTAS_ES.indexOf(n.n);
            let nuevoIdx = oldIdx + semitonosOffset;
            let nuevaOctava = n.o;
            while (nuevoIdx >= 12) { nuevoIdx -= 12; nuevaOctava++; }
            while (nuevoIdx < 0) { nuevoIdx += 12; nuevaOctava--; }
            let nombreEng = NOTAS_EN[nuevoIdx];

            let midi = convertirAMidi(nombreEng, nuevaOctava);
            let closest = archivoWavMasCercanoPitos(midi);
            let rapidez = Math.pow(2, closest.semitonos / 12);
            arr.push(`  {"/Brillante/${closest.wav}.wav", ${rapidez.toFixed(4)}}`);
        } else {
            arr.push(`  {"", 1.0000}`);
        }
    }
...
function archivoWavMasCercanoBajos(nombreEng, tipo) {
    // Implementación simple hardcodeada porque la SD ahora tiene nombres limpios como "BajoEb-cm.wav"
    let wavBuscado = "";
    if (tipo === 'nota') {
        wavBuscado = `Bajo${nombreEng}-cm`;
        // si no existe, usar el generico... 
        return wavsBajosLimpios.includes(wavBuscado) ? wavBuscado : wavsBajosLimpios[1]; // fallback 
    } else {
        let modoStr = tipo === 'menor' ? 'm' : '';
        wavBuscado = `Bajo${nombreEng}${modoStr}(acorde)-cm`;
        return wavsBajosLimpios.includes(wavBuscado) ? wavBuscado : wavsBajosLimpios[0]; // fallback
    }
}
...
function generarArrayBajosC(fila_obj, tipo_fuelle, semitonosOffset) {
    let arr = [];
    for (let i = 1; i <= 6; i++) {
        let n = fila_obj.find(x => x.i === i && x.t === tipo_fuelle);
        if (n) {
            let oldIdx = NOTAS_ES.indexOf(n.n);
            let nuevoIdx = oldIdx + semitonosOffset;
            let nuevaOctava = n.o;
            ...
            let nombreWav = archivoWavMasCercanoBajos(nombreEng, n.tipo);
            arr.push(`  {"/Bajos/${nombreWav}.wav", 1.0000}`);
        } else {
            arr.push(`  {"", 1.0000}`);
        }
    }
...
let output = "";
// REESCRIBIR LOS ARRAYS DE FORMA QUE SE MAPEEN A LOS MUX:
// Importante: tu codigo Serial.print dice: "H1" -> SigMux1
// Si tu codigo viejo enviaba "H1" para sigMux1, y tu web esperaba "primeraFila"
// Vamos a reconstruir todo C++ usando 0 transposición (Original FBE)

output += "// ------------- HILERA 1 (MUX 1) -------------\n";
output += "const ArchivoAudio H1_Empujar[16] = \n" + generarArrayPitosC(DEFINICION_BASE.primeraFila, 'empujar', offset_original) + ";\n\n";
output += "const ArchivoAudio H1_Halar[16] = \n" + generarArrayPitosC(DEFINICION_BASE.primeraFila, 'halar', offset_original) + ";\n\n";

output += "// ------------- HILERA 2 (MUX 2) -------------\n";
output += "const ArchivoAudio H2_Empujar[16] = \n" + generarArrayPitosC(DEFINICION_BASE.segundaFila, 'empujar', offset_original) + ";\n\n";
output += "const ArchivoAudio H2_Halar[16] = \n" + generarArrayPitosC(DEFINICION_BASE.segundaFila, 'halar', offset_original) + ";\n\n";

output += "// ------------- HILERA 3 & BAJOS (MUX 3) -------------\n";
output += "const ArchivoAudio H3_Empujar[16] = \n" + generarArrayPitosC(DEFINICION_BASE.terceraFila, 'empujar', offset_original) + ";\n\n";
output += "const ArchivoAudio H3_Halar[16] = \n" + generarArrayPitosC(DEFINICION_BASE.terceraFila, 'halar', offset_original) + ";\n\n";

// BAJOS no entran por el MUX por ahora segun tu codigo, estan en standby
output += "const ArchivoAudio B_Interiores_Empujar[6] = \n" + generarArrayBajosC(DEFINICION_BASE.bajosUna, 'empujar', offset_original) + ";\n\n";
output += "const ArchivoAudio B_Interiores_Halar[6] = \n" + generarArrayBajosC(DEFINICION_BASE.bajosUna, 'halar', offset_original) + ";\n\n";
output += "const ArchivoAudio B_Exteriores_Empujar[6] = \n" + generarArrayBajosC(DEFINICION_BASE.bajosDos, 'empujar', offset_original) + ";\n\n";
output += "const ArchivoAudio B_Exteriores_Halar[6] = \n" + generarArrayBajosC(DEFINICION_BASE.bajosDos, 'halar', offset_original) + ";\n\n";

const filePath = 'c:/Users/acord/OneDrive/Desktop/Academia2026/src/Paginas/SimuladorDeAcordeon/NuevoMIDI/acordeon_esp32.ino';
let code = fs.readFileSync(filePath, 'utf8');
code = code.replace(/\/\/ ------------- HILERA 1[\s\S]*?B_Exteriores_Halar\[6\] = [\s\S]*?};\n/g, output);

fs.writeFileSync(filePath, code);
console.log("¡Reconstruido Tonalidad Original Fa-Sib-Mib y re-escrito en Arduino!");
