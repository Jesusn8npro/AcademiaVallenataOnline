...
const normalizarTipo = (tipo: string | undefined) => tipo === 'tutorial' ? 'cancion' : (tipo || 'secuencia');
...
const ModalListaHero: React.FC<ModalListaHeroProps> = ({ visible, onCerrar, onReproducir }) => {
    ...
    const [filtro, setFiltro] = useState<'todos' | 'secuencia' | 'cancion' | 'ejercicio'>('todos');
    ...
    const filtradas = filtro === 'todos' ? canciones : canciones.filter(c => normalizarTipo(c.tipo) === filtro);
            ...
            >
                ...
                >
                    ...
                    <div style={{ padding: '12px 24px', display: 'flex', gap: '8px', overflowX: 'auto', borderBottom: '1px solid #1a1a1a' }}>
                        {['todos', 'secuencia', 'ejercicio', 'cancion'].map((f) => (
                            <button
                                key={f}
                                ...
                                >
                                    ...
                                    <div style={{ 
                                        ...
                                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                                         backgroundColor: normalizarTipo(cancion.tipo) === 'cancion' ? 'rgba(239, 68, 68, 0.1)' : 
                                                          normalizarTipo(cancion.tipo) === 'ejercicio' ? 'rgba(34, 197, 94, 0.1)' : 'rgba(0, 229, 255, 0.1)',
                                         color: normalizarTipo(cancion.tipo) === 'cancion' ? '#ef4444' : 
                                                normalizarTipo(cancion.tipo) === 'ejercicio' ? '#22c55e' : '#00e5ff'
                                     }}>
                                         {normalizarTipo(cancion.tipo) === 'cancion' ? <Music size={24} /> : 
                                          normalizarTipo(cancion.tipo) === 'ejercicio' ? <BookOpen size={24} /> : <Activity size={24} />}
                                     </div>
