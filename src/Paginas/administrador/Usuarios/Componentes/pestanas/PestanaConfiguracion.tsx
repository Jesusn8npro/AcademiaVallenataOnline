...
const PestanaConfiguracion: React.FC<Props> = ({ usuario, onUsuarioActualizado }) => {
  ...
  } = usePestanaConfiguracion(usuario, onUsuarioActualizado);
    ...
    <div className="uconf-container">
      ...
      <div className="uconf-section uconf-section-membership">
        <div className="uconf-section-header">
          ...
          <button className="uconf-btn-action" onClick={() => setMostrarGestionMembresia(!mostrarGestionMembresia)} disabled={cargando}>
        ...
        <div className="uconf-membership-card">
          <div className="uconf-membership-icon-wrapper">{getMembresiaIcon(usuario.suscripcion)}</div>
          <div>
      ...
      <div className="uconf-section uconf-section-password">
        <div className="uconf-section-header">
          ...
          <div className="uconf-header-actions">
            <button className="uconf-btn-primary" onClick={() => setMostrarCambioPassword(!mostrarCambioPassword)} disabled={cargandoPassword}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
                <path d="M12 15h2m-6 4h8a2 2 0 002-2V7a2 2 0 00-2-2H8a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              Cambiar Contraseña
            </button>
            <button className="uconf-btn-success" onClick={enviarEmailRestablecimientoHandler} disabled={cargandoEmail}>
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          <div className="uconf-password-form">
            ...
            <div className="uconf-form-actions">
              ...
              <button
                ...
                onClick={() => { setMostrarCambioPassword(false); setNuevaPassword(''); setConfirmarPassword(''); }}
                disabled={cargandoPassword}
              >
                Cancelar
              </button>
            ...
            {historialPagos.map((pago: Pago) => (
              <div key={pago.id} className="uconf-payment-item">
                ...
                <div className="uconf-payment-right">
                  ...
                  <span className={`uconf-payment-status uconf-status-${pago.estado.toLowerCase()}`}>{pago.estado}</span>
                </div>
              </div>
            ))}
          </div>
