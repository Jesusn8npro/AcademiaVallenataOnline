...
NUEVOS = [
    ("Sentarse.fbx", "Sentarse"),
    ("Subiendo escaleras.fbx", "Subiendo escaleras"),
]
...
TRIM = {}
# Clips de LOCOMOCIÓN en el sitio (sin subir/bajar la cadera por el clip): la altura la pone la FÍSICA
# del juego (detección de piso). 'Subiendo escaleras' acumula subida en el clip → si se conserva, el
# cuerpo sube DOBLE (clip + piso). Por eso se le quita la vertical (queda el paso en el sitio). 'Sentarse'
# NO va aquí: necesita su bajada vertical real (el cuerpo baja al sentarse).
SIN_VERTICAL = {"Subiendo escaleras"}
...
def agregar_baile(filepath, nombre, rig):
    ...
    # Locomoción en el sitio: descartar la vertical del clip (la pone la física del piso) → sin doble subida.
    if nombre in SIN_VERTICAL:
        computedLoc = {}
